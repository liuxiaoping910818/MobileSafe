...
import liu.com.mobilesafe.dbutil.BlackNumberOpenHelper;
import liu.com.mobilesafe.domain.BlackNumberInfo;
    ...
    public static BlackNumberDao getInstance(Context ctx) {
        if (sInstance == null) {
            ...
            synchronized (BlackNumberDao.class) {
                if (sInstance == null) {
                    sInstance = new BlackNumberDao(ctx);//程序执行到此处，在其内部进行着三个步骤；1.给sInstance分全内存，2调用sInstance的构造函数 ；3，将sInstance对象指向分配的空间
                }
            }
        }
