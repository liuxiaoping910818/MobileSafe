...
import liu.com.mobilesafe.dao.AntiVirusDao;
...
public class AntiVirusActivity extends AppCompatActivity {
    ...
    private Handler mHandler = new Handler() {
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                case STATE_UPDATE_STATUS://更新扫描状态
        ...
        new Thread() {
            public void run() {
                ...
                List<PackageInfo> installedPackages = pm
                        .getInstalledPackages(PackageManager.GET_UNINSTALLED_PACKAGES);//获取集合

                pbProgress.setMax(installedPackages.size());//设置最大值
                int progress = 0;
                Random random = new Random();
                for (PackageInfo packageInfo : installedPackages) {//循环遍历包集合。
                    ScanInfo info = new ScanInfo();

                    String packageName = packageInfo.packageName;//拿到应用的包名
                    String name = packageInfo.applicationInfo.loadLabel(pm)
                            .toString();//通过包名拿到应用的名称
                    ...
                    String md5 = MD5Utils.encode(apkPath);
                    ...
                    // 更新扫描状态
                    Message msg = Message.obtain(); //=================>将消息发送到主线程，主线程实时刷新。
